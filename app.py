...
# --- GİRİŞ EKRANI YÖNETİMİ (SESSION STATE) ---
if 'secim_cevabi' not in st.session_state:
    st.session_state.secim_cevabi = None

def evet_secildi():
    st.session_state.secim_cevabi = "evet"

def hayir_secildi():
    st.session_state.secim_cevabi = "hayir"

def geri_don():
    st.session_state.secim_cevabi = None

# 1. DURUM: UYGULAMA İLK AÇILDIĞINDA SORU EKRANI
if st.session_state.secim_cevabi is None:
    st.markdown("<br><br><br>", unsafe_allow_html=True)
    st.markdown("""
    <div style="text-align: center; background-color: #f8f9fa; padding: 40px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); border-top: 6px solid #1a73e8; border-bottom: 6px solid #1a73e8;">
        <h1 style="color: #202124; font-size: 34px; font-weight: 900; margin-bottom: 10px;">SEÇİMİ KAZANACAK MIYIZ?</h1>
    </div>
    <br><br>
    ...
    with col1:
        st.button("EVET 🚀", on_click=evet_secildi, use_container_width=True, type="primary")
    with col2:
        st.button("HAYIR 👎", on_click=hayir_secildi, use_container_width=True)

# 2. DURUM: "HAYIR" SEÇİLDİĞİNDE ÇIKACAK EKRAN
elif st.session_state.secim_cevabi == "hayir":
    st.markdown("<br><br><br>", unsafe_allow_html=True)
    st.markdown("""
    <div style="background-color: #dc3545; padding: 50px; border-radius: 15px; text-align: center; box-shadow: 0 10px 25px rgba(220,53,69,0.4);">
        <h1 style="color: white; font-size: 55px; font-weight: 900; letter-spacing: 3px; margin: 0;">DEVRİK BAŞKAN!</h1>
    </div>
    <br><br>
    """, unsafe_allow_html=True)
    
    # Kullanıcının hatasını anlayıp geri dönmesi için küçük bir buton :)
    st.button("Geri Dön ve Doğru Cevabı Ver", on_click=geri_don, use_container_width=True)

# 3. DURUM: "EVET" SEÇİLDİĞİNDE ÇIKACAK ANA UYGULAMA EKRANI
elif st.session_state.secim_cevabi == "evet":
